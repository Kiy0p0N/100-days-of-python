...
def save_information():
    ...
    try:
        ...
        with open('data.json', mode='w') as data_file:
            json.dump(user_information, data_file, indent=4)

    else:
        data.update(user_information)  # Updating the file
        with open('data.json', mode='w') as data_file:
...
def search():
    """
    Search for user information on a specific website.
    """
    ...
    except FileNotFoundError:  # Display an error message if the 'data.json' file does not exist
        messagebox.showerror(title="Error", message="This file was not found.")

    else:
        if website in data_json:
            data = data_json[website]
            message = f"""
            ...
            # Display an error message if the 'website' key does not exist
            messagebox.showerror(title="Error", message="This key was not found.")
...
def generate_password():
    ...
    password_entry.delete(0, END)  # Deletes the last password before creating another one
...
website_btn = Button(text="Search", command=search, width=14)
website_btn.grid(column=2, row=1)
...
password_btn = Button(text="Generate Password", command=generate_password, width=14)  # Button to generate a random password
